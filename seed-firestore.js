/**
 * Firestore Database Seeding Script for Abhishek Sonar's Portfolio
 * 
 * Instructions:
 * 1. Place your Firebase Admin SDK service account key file as `serviceAccountKey.json` in the root directory.
 * 2. Run: `node seed-firestore.js`
 */
...
if (!fs.existsSync(serviceAccountPath)) {
  ...
  console.log('Save it as `serviceAccountKey.json` in the root of this project and run this script again.');
  process.exit(1);
}
...
const skillsData = [
  ...
  { name: "Git & Version Control", category: "Libraries & Tools", iconClass: "devicon-git-plain", proficiency: 90 },
  { name: "Docker", category: "Libraries & Tools", iconClass: "devicon-docker-plain", proficiency: 80 },
  
  // Core Competencies
  { name: "Micro-Frontend Architecture", category: "Core Competencies", iconClass: "devicon-custom-mfe", proficiency: 92 },
  { name: "API Integration (REST/WS)", category: "Core Competencies", iconClass: "devicon-custom-api", proficiency: 95 },
  { name: "Unit Testing (Vitest/Jest)", category: "Core Competencies", iconClass: "devicon-custom-test", proficiency: 88 },
  { name: "Agile / Scrum / Jira", category: "Core Competencies", iconClass: "devicon-jira-plain", proficiency: 90 },
  { name: "Code Review & Mentorship", category: "Core Competencies", iconClass: "devicon-custom-reviews", proficiency: 95 },
...
const educationData = [
  {
    institution: "Guru Gobind Singh College of Engg., Nashik, Maharashtra",
  ...
  {
    institution: "Govt. Polytechnic, Nandurbar, Maharashtra",
...
async function seed() {
  ...
  await db.collection('profile').doc('default').set(profileData);
  
  // Seed Skills
  console.log('Seeding skills...');
  const skillsCol = db.collection('skills');
  // Clear old skills
  const oldSkills = await skillsCol.get();
  for (const doc of oldSkills.docs) {
    await doc.ref.delete();
  }
  for (const skill of skillsData) {
    await skillsCol.add(skill);
  }
  
  // Seed Experience
  console.log('Seeding experience...');
  const expCol = db.collection('experience');
  const oldExp = await expCol.get();
  for (const doc of oldExp.docs) {
    await doc.ref.delete();
  }
  // Let's add them in order to keep sorting simple, or add a sortIndex
  for (let i = 0; i < experienceData.length; i++) {
    await expCol.add({ ...experienceData[i], sortIndex: i });
  }
  
  // Seed Education
  console.log('Seeding education...');
  const eduCol = db.collection('education');
  const oldEdu = await eduCol.get();
  for (const doc of oldEdu.docs) {
    await doc.ref.delete();
  }
  for (let i = 0; i < educationData.length; i++) {
    await eduCol.add({ ...educationData[i], sortIndex: i });
  }
  
  // Seed Projects
  console.log('Seeding projects...');
  const projCol = db.collection('projects');
  const oldProj = await projCol.get();
  for (const doc of oldProj.docs) {
    await doc.ref.delete();
  }
  for (let i = 0; i < projectsData.length; i++) {
    await projCol.add({ ...projectsData[i], sortIndex: i });
  }
